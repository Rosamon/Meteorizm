...
int Game::run() {
    ...
    while (running && isntOver) {
        ...
        while (window.pollEvent(event)) {
            ...
            Sprite GameEnd;
            GameEnd.setTexture(Texture_End);//ïåðåäà¸ì â íåãî îáúåêò Texture (òåêñòóðû)
            window.draw(GameEnd);
            window.display();
            Sleep(5000);
            return EXIT_SUCCESS;
...
void Game::createGameWindow() {
    ...
    bool isMenu = 1;
    int menuNum = 0;
    GamePic.setTexture(Texture_Start);//Ã¯Ã¥Ã°Ã¥Ã¤Ã Â¸Ã¬ Ã¢ Ã­Ã¥Ã£Ã® Ã®Ã¡ÃºÃ¥ÃªÃ² Texture (Ã²Ã¥ÃªÃ±Ã²Ã³Ã°Ã»)
    ...
    while (isMenu)
    {
        ...
        {
            if (menuNum == 2) { window.close(); system("exit"); };//Ã¥Ã±Ã«Ã¨ Ã­Ã Ã¦Ã Ã«Ã¨ Ã¯Ã¥Ã°Ã¢Ã³Ã¾ ÃªÃ­Ã®Ã¯ÃªÃ³, Ã²Ã® Ã¢Ã»ÃµÃ®Ã¤Ã¨Ã¬ Ã¨Ã§ Ã¬Ã¥Ã­Ã¾ 
